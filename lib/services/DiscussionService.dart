...
class DiscussionService {
  ...
  Future<List<Discussion>> getDiscussionsByDeclarationId(String declarationId) async {
    List<Discussion> discussions = [];
    try {
      http.Response response = await http.get(
        Uri.parse("${API.URL}/api/discussions/declarations/$declarationId/discussions"),
        headers: {
  ...
  Future<List<Message>> getMessagesForDiscussion(String declarationId, String discussionId) async {
    List<Message> messages = [];
    try {
      http.Response response = await http.get(
        Uri.parse("${API.URL}/api/declarations/$declarationId/discussions/$discussionId/messages"),
        headers: {
          HttpHeaders.contentTypeHeader: 'application/json',
        },
      ).timeout(Duration(seconds: 10));
  ...
  Future<Message?> sendMessage({
    ...
    try {
      ...
      http.Response response = await http.post(
        Uri.parse("${API.URL}/api/messages"),
        headers: {
          HttpHeaders.contentTypeHeader: 'application/json',
        },
  ...
  Future<Discussion?> createDiscussion({
    ...
    try {
      ...
      http.Response response = await http.post(
        Uri.parse("${API.URL}/api/discussions"),
        headers: {
          HttpHeaders.contentTypeHeader: 'application/json',
        },
